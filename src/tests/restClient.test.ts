...
describe('RestClient', () => {
  beforeEach(() => {
    global.fetch = vi.fn(async () => new Response(JSON.stringify({ time: Date.now() })))
  })
  ...
  it('constructor enforces HTTPS', () => {
    const am = new AuthManager(new Provider(), new Store())
    expect(() => new RestClient('http://api', am)).toThrow('Base URL must be HTTPS')
  })
  ...
  it('post and delete methods wrap request correctly', async () => {
    ...
    global.fetch = vi.fn(async (_url, init) =>
      new Response(JSON.stringify(init?.body), { status: 200, headers: { 'x-ratelimit-reset': '0' } })
    )
    const postRes = await rc.post<{ foo: string }>('/p', payload)
    expect(postRes).toEqual(JSON.stringify(payload))
    // test DELETE
    global.fetch = vi.fn(async () =>
      new Response(JSON.stringify({ foo: 'del' }), { status: 200, headers: { 'x-ratelimit-reset': '0' } })
    )
